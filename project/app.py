...
from helpers import apology, login_required, usd, allowed_file, process_chase_transactions, process_discover_transactions, process_citi_transactions, process_noble_transactions, update_current_value, get_total_cash, get_total_debt, calculate_net_worth, get_account_data
...
def index():
    ...
    for category in index_data:
        category["percentage_spent"] = (Decimal(category["budget_spent"]) / (total_budget_spent if total_budget_spent != Decimal(0) else Decimal(1))) * 100
...
def accounts():
    ...
    if request.method == "GET":
        # Update current value of accounts
        update_current_value(db, user_id)

        # Get total cash, total debt, and net worth
        total_cash = get_total_cash(db, user_id)
        total_debt = get_total_debt(db, user_id)
        net_worth = calculate_net_worth(total_cash, total_debt)

        # Get account data
        account_data = get_account_data(db, user_id)
        return render_template("accounts.html", account_data=account_data, total_cash=total_cash, total_debt=total_debt, net_worth=net_worth, usd=usd)
...
def delete_transactions():
    ...
    if request.method == "POST":
        ...
        total_cash = get_total_cash(db, user_id)
        total_debt = get_total_debt(db, user_id)
        calculate_net_worth(total_cash, total_debt)
